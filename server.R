...
# Add column which says whether the movie won any Oscars
  ...
  vis <- reactive({
    # Lables for the x and y axes that will vary upon users selection
    xvariable_name <- names(axis_variables)[axis_variables == input$xvariable]
    yvariable_name <- names(axis_variables)[axis_variables == input$yvariable]

    xvariable <- prop("x", as.symbol(input$xvariable))
    yvariable <- prop("y", as.symbol(input$yvariable))

  #creating a tooltip that will include the movie name, the year it was made and dollars at box office
    mov %>%
      ggvis(x = xvariable, y = yvariable) %>%
      add_tooltip(movie_tooltip, "hover") %>%
      layer_points(size := 50, size.hover := 300,
                   fillOpacity := 0.2, fillOpacity.hover := 1,
                   stroke = ~has_oscar, key := ~ID) %>%
      add_axis("x", title = xvariable_name) %>%
      add_axis("y", title = yvariable_name) %>%
